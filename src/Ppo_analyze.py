...
Reads multiple PPO output dirs, each containing proxy_log.json (per-step
proxy reward) and gold_log.json (per-step gold reward from offline eval).
Produces a two-panel figure mirroring the PhD's RL transfer figure:
left = proxy reward over PPO updates, right = gold reward.

Run from project root:
    python -m src.ppo_analyze \
        --ppo_dirs ./outputs/ppo-ident-no-kl ./outputs/ppo-ident-kl \
                   ./outputs/ppo-gaussian-no-kl ./outputs/ppo-gaussian-kl \
        --labels "Linear no-KL" "Linear KL" "Gaussian no-KL" "Gaussian KL" \
...
def main():
    ...
    parser.add_argument("--smooth_window", type=int, default=10,
                        help="Rolling window for proxy reward smoothing.")
    ...
    for label, ppo_dir in zip(args.labels, args.ppo_dirs):
        ...
        if proxy_log:
            ...
            if args.smooth_window > 1 and len(proxy) >= args.smooth_window:
                ...
                axes[0].plot(steps, proxy, alpha=0.3, label=None)
    ...
    axes[0].set_xlabel("PPO updates")
    axes[0].set_ylabel("Proxy reward (training signal)")
    axes[0].set_title("Proxy reward over PPO updates")
    axes[0].legend()
    axes[0].grid(alpha=0.3)
    
    axes[1].set_xlabel("PPO updates")
    axes[1].set_ylabel("Gold reward (Skywork)")
    axes[1].set_title("Gold reward over PPO updates")
    axes[1].legend()
